...
const weatherAPIURL = "https://api.openweathermap.org/data/2.5/weather?q=Curitiba&appid=2838e61f6d31a85f3212a3c686b49e64&units=metric";
const forecastAPIURL = "https://api.openweathermap.org/data/2.5/forecast?q=Curitiba&appid=2838e61f6d31a85f3212a3c686b49e64&units=metric&cnt=24";
...
// Current Weather
async function apiFetch(url, current) { // false for weather forecast, true for current weather
    try {
...
function displayCurrentWeather({ main: { temp }, weather: [{ main, description, icon }] }) {
    ...
    currentTemperature.innerHTML = `${temp.toFixed(0)}&deg;C`;
...
function displaySpotlightsAds({ companies }) {
    const spotlightCompanies = companies.filter(({ name }) =>
        name === "Senepar" || name === "Copel" || name === "Prefeitura de Curitiba");

    const displayListCompanies = [];
    const copySpotlightCompanies = [...spotlightCompanies];

    for (let i = 0; i < 3; i++) {
        const randomIndex = Math.floor(Math.random() * copySpotlightCompanies.length);
        displayListCompanies.push(copySpotlightCompanies[randomIndex]);
        copySpotlightCompanies.splice(randomIndex, 1);
    }

    for (let i = 0; i < displayListCompanies.length; i++) {
...
// Simulated data for Senepar, Copel, and Prefeitura de Curitiba (replace with actual data)
const companiesData = {
    companies: [
        {
            name: "Senepar",
            address: "Av. Paraná, 3000 - Curitiba, PR",
            phone: "+55 41 1234-5678",
            url: "http://www.senepar.com.br",
            membershipLevel: "Gold Membership"
        },
        {
            name: "Copel",
            address: "Rua Coronel Dulcídio, 800 - Curitiba, PR",
            phone: "+55 41 8765-4321",
            url: "http://www.copel.com",
            membershipLevel: "Silver Membership"
        },
        {
            name: "Prefeitura de Curitiba",
            address: "Rua da Cidadania do Boqueirão, s/n - Curitiba, PR",
...
// Display spotlights ads for Senepar, Copel, and Prefeitura de Curitiba
displaySpotlightsAds(companiesData);
